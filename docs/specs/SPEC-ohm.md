...
**C13 — Task 0 auth gate is proven by HTTP 200, not row count.** The empty pre-deploy org may have 0 `ApexClass` rows; auth is proven by a `200` from `callout:Ohm_Self` on the Tooling `query` of **`GenAiPlannerDefinition`** returning `records:[]`. Row-bearing queries are optional secondary confirmations only. (Resolves the conflation of auth with data presence.)
